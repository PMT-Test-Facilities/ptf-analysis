...
//double mufix = Nrest / N0;                                                                                                                                                         
 ...
 for(unsigned int iscan=0; iscan<N; iscan++){
	...
	continue;
     ...
     std::cout<<"TEST scan point "<<iscan<<" inside circle " <<std::endl;
     
  PMTResponsePed::set_binwid( histos_fitted[iscan]->GetBinWidth(1) );
  TF1* ftmp = new TF1( "pmt_response", model1, 0., 5000., 6 );
  double curNfix  =  histos_fitted[iscan]->Integral(1,  histos_fitted[iscan]->GetNbinsX()+1, "width" );
  double curN0    =  histos_fitted[iscan]->Integral(1,1);
  double curNrest =  histos_fitted[iscan]->Integral(2, histos_fitted[iscan]->GetNbinsX()+1 );
  ...
  cout<<ftmp->GetParameter(4)<<endl;
  cout<<ftmp->GetParameter(5)<<endl;
